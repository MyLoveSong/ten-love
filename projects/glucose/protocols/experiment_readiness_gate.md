...
| result summary | lightweight JSON or Markdown table, no raw data, no checkpoints, no row-level predictions | old-candidate summaries and BigIdeas baseline summary created; claim remains local |
...
- The gate remains not passed until BigIdeas GluFormer 30-epoch multi-seed comparison, multi-seed policy, final claim-specific Data Availability wording, and claim-boundary review all pass.
